...
import org.tommy.stationery.moracle.core.domain.SqlResults;
 ...
 */
public class MoracleResultSet  implements ResultSet {

    private static final Logger logger = LoggerFactory.getLogger(MoracleResultSet.class);

    int _row = 0;
    boolean _closed = false;
    int curser = -1;
    MoracleReturnData data;
    SqlResults sqlResults;
    Map<String, Integer> mapColumns = new HashMap<String, Integer>(100000);

    MoracleResultSet(MoracleReturnData data) throws IOException {
        this.data = data;

        curser = -1;
        _row = 0;

        //hashing columns index;
        List<MColumn> columns = data.getColumns();
        int index = 0;
        for (MColumn column : columns) {
            mapColumns.put(column.getName(), index);
            index++;
        }

        String filePath = data.getUrl();
        String fileEncoding = data.getConfig().getString(ConfigEnum.fileEncoding);
        String seperator = data.getConfig().getString(ConfigEnum.seperator);
        ...
        while ((rawRow = in.readLine()) != null) {
            ...
            sqlResults.addRaw(Arrays.<String>asList(rows));
        }
    ...
    public ResultSetMetaData getMetaData(){
        return new ResultSetMetaData() {
            ...
            public String getColumnLabel(int i) throws SQLException {
                return data.getColumns().get(i-1).getName();
            }
    ...
    public String getString(int columnIndex){
        ...
        //DEBUG
        if (sqlResults.getListFromCursor().get(curser).size() < columnIndex) {
            return "";
        }
        String val = sqlResults.getListFromCursor().get(curser).get(columnIndex);
        if ( val == null )
            return "";
        return val;
    }
    ...
    public String getString(String columnName){
        ...
        String val = sqlResults.getListFromCursor().get(curser).get(findColumn(columnName));
        if ( val == null )
            return "";
        return val;
    }
    ...
    public int findColumn( String columnName ){
        return mapColumns.get(columnName);
    }
    ...
    public boolean next(){
        ...
        if (sqlResults.getListFromCursor().size()-1 <= curser) {
            curser = -1;
            sqlResults = null;
            return false;
        }
